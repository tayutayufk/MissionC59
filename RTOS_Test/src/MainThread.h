...
void IRAM_ATTR MainThread::GetData(){
    ...
    //Scale Adjusting Acc 16G Gyro 2500dps to 1G(float) 1 rad(float)
    mpuDataAcc[0] = mpuDataInt[0] * 0.00048828125f;
    mpuDataAcc[1] = mpuDataInt[1] * 0.00048828125f;
    mpuDataAcc[2] = mpuDataInt[2] * 0.00048828125f;

    mpuDataGyro[0] = (mpuDataInt[3] - mpuGyroCalib[0]) * 0.0762939f * 3.141592 / 180.0f;
    mpuDataGyro[1] = (mpuDataInt[4] - mpuGyroCalib[1]) * 0.0762939f * 3.141592 / 180.0f;
    mpuDataGyro[2] = (mpuDataInt[5] - mpuGyroCalib[2]) * 0.0762939f * 3.141592 / 180.0f;
}
int IRAM_ATTR MainThread::launchDetection(){
    static int Ite = 0;
    static float aveAcc = 0;

    float AccScal = mpuDataAcc[0]*mpuDataAcc[0] + mpuDataAcc[1]*mpuDataAcc[1] + mpuDataAcc[2]*mpuDataAcc[2];
